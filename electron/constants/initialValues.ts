// 这里集中维护“主进程侧可调的初始�?默认值”：
// - 主进程涉及窗口尺寸、快捷键、索引策略等，初始值散落会导致改动容易�?
// - 严格保持与现有逻辑一致：仅抽离常量，不改变原有功�?
...
// 说明�?
// - BASE_SEARCH_TYPE_IDS / DEFAULT_SETTINGS 属于“渲染进�?+ 主进程必须一致”的默认�?
// - 统一�?shared/initialValues.ts 引入，避免维护两份导致漂�?
export {
...
// 搜索窗口初始高度：首次展示只展示输入框（后续由渲染进程动�?resize�?
...
// 索引版本：结构变更时递增，用于触发重�?
export const FILE_INDEX_VERSION = 3;
