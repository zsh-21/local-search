...
export function searchSettingsItems(input: {
  ...
  if (searchTypeId !== "all" && searchTypeId !== "settings") {
    return { settingsResults: [], settingsOnly: null as any };
  }

  const out = settingsItems.map((it) => ({
    name: it.name,
    path: it.uri,
    type: "settings",
    source: "settings",
  ...
  if (searchTypeId === "settings") {
    return { settingsResults: [], settingsOnly: { results: out, totalCount: out.length } };
  }

  return { settingsResults: out, settingsOnly: null };
}
