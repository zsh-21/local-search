...
export class RecursiveScanner implements Scanner {
  ...
  private async processDirectory(
    ...
    try {
        const entries: Array<{ dirent: Dirent; fullPath: string; isDirectory: boolean; ext: string }> = [];
        for await (const dirent of dir) {
            if (shouldStop()) break;
            /** 符号链接可能导致死循环或指向外部，这里直接跳过。 */
