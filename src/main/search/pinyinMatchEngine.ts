...
// 构建严格匹配候选文本：名称/别名按前缀匹配，首字母串按序列匹配
...
function scoreCandidates(input: {
  ...
  const strictNameScorer = createNameScorer(input.query, {
    ...
    // 应用/设置场景保持现有策略，不额外启用 disableFuzzyFallback，避免引入行为变化。
  });
...
function toScoreDebugRow(item: RankedCandidate): SearchScoreDebugRow {
  const d = item.scoreDetail && typeof item.scoreDetail === "object"
    ? (item.scoreDetail as Record<string, unknown>)
    : {};
  return {
