...
function registerDevtoolsHotkey(getWindow: () => BrowserWindow | null) {
...
export function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    icon: resolvePublicAssetPath('tray.png'),
    webPreferences: {
      preload: path.join(__dirname, 'preload.js'),
    },
  });
  registerDevtoolsHotkey(() => win);
  win.on('moved', () => {
  ...
  win.on('blur', () => {
    ...
    searchHideTimer = setTimeout(() => {
      ...
      if (win.isVisible()) {
        try {
          win.webContents.send(IPC_EVENT_SEARCH_WINDOW_HIDDEN);
        } catch {}
        win.hide();
      }
...
export function createSettingsWindow() {
  ...
  registerDevtoolsHotkey(() => settingsWin);
  ...
  settingsWin.once('ready-to-show', () => {
    ...
    settingsShowFallbackTimer = setTimeout(() => {
      ...
      settingsWin.show();
      settingsWin.focus();
      settingsWin.webContents.send(IPC_EVENT_SETTINGS_WINDOW_OPENED);
    }, 1200);
  });
...
export function openSearchWindow() {
  ...
  const sendOpenEvent = () => {
    ...
    if (!win || win.isDestroyed()) return;
    try {
      if (settings.keepStateOnClose) win.webContents.send(IPC_EVENT_SEARCH_WINDOW_OPENED);
      else win.webContents.send(IPC_EVENT_RESET_SEARCH);
    } catch {}
  };
  const showWhenReady = () => {
...
export async function handleQuickItemPicked(targetPath: string) {
  try {
    ...
    win?.webContents.send(IPC_EVENT_HISTORY_UPDATED, { results });
    settingsWin?.webContents.send(IPC_EVENT_HISTORY_UPDATED, { results });
    win?.webContents.send(IPC_EVENT_RESET_SEARCH);
  } catch {}
}
