...
import { IPC_GET_IMAGE_DATA_URL } from "../../shared/ipc/channels";
type GetImageDataUrlResponse = { ok?: boolean; dataUrl?: string };
export function BackgroundImage({ path, opacity }: { path: string; opacity: number }) {
  ...
  useEffect(() => {
    ...
    window.ipcRenderer
      ?.invoke(IPC_GET_IMAGE_DATA_URL, normalizedPath)
      .then((raw) => {
        const resp = raw as GetImageDataUrlResponse | undefined;
