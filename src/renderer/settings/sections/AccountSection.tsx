...
import { IPC_GET_IMAGE_DATA_URL, IPC_SELECT_AVATAR_IMAGE } from "../../../shared/ipc/channels";
...
export function AccountSection({
  ...
  useEffect(() => {
    ...
    window.ipcRenderer
      ?.invoke(IPC_GET_IMAGE_DATA_URL, p)
      .then((resp: unknown) => {
        if (requestIdRef.current !== requestId) return;
        const parsed = resp && typeof resp === "object" ? (resp as { ok?: boolean; dataUrl?: unknown }) : undefined;
        if (parsed?.ok && typeof parsed.dataUrl === "string") setAvatarDataUrl(parsed.dataUrl);
      })
      .catch(() => {});
                  ...
                  <div className="avatar-menu" onMouseDown={(e) => e.stopPropagation()}>
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setError("");
                        const resp = (await window.ipcRenderer?.invoke(IPC_SELECT_AVATAR_IMAGE)) as
