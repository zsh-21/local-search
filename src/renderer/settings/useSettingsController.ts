...
export function useSettingsController() {
  ...
  useEffect(() => {
    ...
    window.ipcRenderer?.invoke(IPC_SETTINGS_VIEW_READY);
  ...
  useEffect(() => {
    ...
    window.ipcRenderer?.on(IPC_EVENT_SETTINGS_WINDOW_OPENED, handler);
    return () => {
      window.ipcRenderer?.off(IPC_EVENT_SETTINGS_WINDOW_OPENED, handler);
    };
  }, [settings]);
  ...
  const onToggleMax = async () => {
    const resp = (await window.ipcRenderer?.invoke(IPC_TOGGLE_MAXIMIZE)) as
      | { maximized: boolean }
      | undefined;
  ...
  const save = async () => {
    ...
    const resp = (await window.ipcRenderer?.invoke(IPC_SAVE_SETTINGS, nextDraft)) as
  ...
  const clearAvatarSettingIfNeeded = async () => {
    ...
    try {
      ...
      await window.ipcRenderer?.invoke(IPC_SAVE_SETTINGS, next);
    } catch {
  ...
  const fontOptions = SETTINGS_FONT_OPTIONS;
